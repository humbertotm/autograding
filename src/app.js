...
app.post('/compile', upload.single('codefile'), function(req, res) {
  ...
  var identifier = toString(Math.floor(Math.random() * 1000000));
  ...
  var dirToCopy = path.join(__dirname, './usercode/' + compilersArr[langId][0]);
  ...
  exec(compSt, function(err, stdOut, stdErr) {
    ...
    if(err) {
      console.log('Some error occured while compiling.');
    } else if(stdErr) {
